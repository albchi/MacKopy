...
class host_data extends uvm_sequence_item;
  ...
  function new(string name="host_data");
    super.new(name);
    `uvm_info("Trace", $psprintf("%m"), UVM_HIGH);
  endfunction
endclass
...
class reg2host_adapter extends uvm_reg_adapter;
  `uvm_object_utils(reg2host_adapter)

  function new(string name="reg2host_adapter");
    super.new(name);
    `uvm_info("Trace", $psprintf("%m"), UVM_HIGH);
  endfunction
  ...
  virtual function uvm_sequence_item reg2bus(const ref uvm_reg_bus_op rw);
    ...
    //
    // Then, populate the reg2bus methods to converting the UVM register data to the host_data type.
    //
    // ToDo
