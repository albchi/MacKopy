...
class host_ral_sequence extends uvm_reg_sequence #(uvm_sequence #(host_data));
  ...
  virtual task body();
    ...
    regmodel.PORT_LOCK.read(.status(status), .value(data), .path(UVM_FRONTDOOR), .parent(this));
    `uvm_info("RAL READ", $sformatf("PORT_LOCK= %2h", data), UVM_MEDIUM);
    regmodel.vreg_rw.write(0,status,data);
    regmodel.vreg_rw.write(10,status,data);
...
//
// The following host sequence doesn't do anything.
//
// Unfortunately this is required in UVM 1.0 because of the lack of ability to
// disable the sequencer once the sequencer's default_sequence field has been configured.
//
// This noop sequence is intended to be used to disable the host sequencer by configuring
// the host sequencer's default_sequence field to this sequence.
//

class host_noop_sequence extends uvm_sequence #(host_data);
  `uvm_object_utils(host_noop_sequence)

  function new(string name = "host_noop_sequence");
    super.new(name);
    `uvm_info("TRACE", $sformatf("%m"), UVM_HIGH);
  endfunction
  task body();
