...
/* ova bool CLRW: inst == 12'b0000_0100_0000;
  clock posedge clk {
event e_clrw: if ( CLRW ) then  #1 w == 8'hab ;
}
assert a_clrw: check(e_clrw);
*/
...
//ova check_bool( expdout == dbus , "HI!!!!! it me Ms Inline" );
...
always @(inst or aluz) begin
   casex ({inst, aluz}) // synopsys parallel_case
      13'b10??_????_????_?:    // A GOTO, CALL or RETLW instructions
         //begin $display("ITS A CALL, inst=%b",inst);
	 skip = 1'b1; //end
...
always @(posedge clk) begin
   if (reset) begin
      ...
      if (wwe) begin
//$display("before assign w=%b aluout=%b", w, aluout);
         w <= dbus;
//$display("after assign w=%b aluout=%b", w, aluout);
      end
   end   
end
...
//$display("Selected portb as output +++++++++++++++++++++");
         portb <= dbus;
      end
   end   
end
...
//$display("Selected portc as output ----------------------- fileaddr=%b", fileaddr[2:0]);
         portc <= dbus;
      end
   end   
end
...
always @(posedge clk) begin
   if (reset) begin
      ...
      //$display(" w=%b", w);
      //$display(" F=%b, SP=%b, FA=%h, ISTRIS=%b, DBUS=%b ", fwe, specialsel, fileaddr[2:0], istris, dbus);
         trisa <= dbus;
         //$display(" trisa=%b", trisa);
      end
   end   
end
