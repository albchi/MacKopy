...
module basic_reg(intf_wb_top.regfile rintf,input bit i_breg_enable, output logic [31:0] o_breg_reg);

logic [31:0] l_basic_reg;
logic [31:0] l_data;
logic l_wr;
logic l_rd;
...
assign o_breg_reg = (l_rd) ? l_basic_reg & p_read & p_read_clear : l_basic_reg & p_read ;
...
begin
	if (rintf.i_reset)
	begin
		l_basic_reg <= p_reset_reg;
	end
	else
	if (i_breg_enable && l_wr)
	begin
		l_basic_reg <= l_data & p_write;

	end

	if (i_breg_enable && l_rd)
	begin
		l_basic_reg <= l_basic_reg & p_read & p_read_clear;
	end
