...
task reset;
begin
   RX[0].gr.atm_rx.reset;
   TX[0].gt.atm_tx.reset;
   disable SWITCH_LOGIC[0].input_port;
   RX[1].gr.atm_rx.reset;
   TX[1].gt.atm_tx.reset;
   disable SWITCH_LOGIC[1].input_port;
   RX[2].gr.atm_rx.reset;
   TX[2].gt.atm_tx.reset;
   disable SWITCH_LOGIC[2].input_port;
   RX[3].gr.atm_rx.reset;
   TX[3].gt.atm_tx.reset;
   disable SWITCH_LOGIC[3].input_port;
   RX[4].gr.atm_rx.reset;
   TX[4].gt.atm_tx.reset;
   disable SWITCH_LOGIC[4].input_port;
   RX[5].gr.atm_rx.reset;
   TX[5].gt.atm_tx.reset;
   disable SWITCH_LOGIC[5].input_port;
   RX[6].gr.atm_rx.reset;
   TX[6].gt.atm_tx.reset;
   disable SWITCH_LOGIC[6].input_port;
   RX[7].gr.atm_rx.reset;
   TX[7].gt.atm_tx.reset;
...
generate
  ...
  for (gv = 0; gv <= 7; gv = gv + 1) begin: RX
     case(gv) 
	   0: begin: gr utopia_atm_rx atm_rx(rx_0); end
	   1: begin: gr utopia_atm_rx atm_rx(rx_1); end
	   2: begin: gr utopia_atm_rx atm_rx(rx_2); end
	   3: begin: gr utopia_atm_rx atm_rx(rx_3); end
	   4: begin: gr utopia_atm_rx atm_rx(rx_4); end
	   5: begin: gr utopia_atm_rx atm_rx(rx_5); end
	   6: begin: gr utopia_atm_rx atm_rx(rx_6); end
	   7: begin: gr utopia_atm_rx atm_rx(rx_7); end
     endcase
  end
endgenerate
...
generate
  ...
  for (gv = 0; gv <= 7; gv = gv + 1) begin: TX
     case(gv) 
	   0: begin: gt utopia_atm_tx atm_tx(tx_0); end
	   1: begin: gt utopia_atm_tx atm_tx(tx_1); end
	   2: begin: gt utopia_atm_tx atm_tx(tx_2); end
	   3: begin: gt utopia_atm_tx atm_tx(tx_3); end
	   4: begin: gt utopia_atm_tx atm_tx(tx_4); end
	   5: begin: gt utopia_atm_tx atm_tx(tx_5); end
	   6: begin: gt utopia_atm_tx atm_tx(tx_6); end
	   7: begin: gt utopia_atm_tx atm_tx(tx_7); end
     endcase
  end
endgenerate
...
generate
  ...
  for (gv = 0; gv <= 7; gv = gv + 1) begin: SWITCH_LOGIC
	...
	begin: input_port
	   ...
	   RX[gv].gr.atm_rx.get_cell(RX[gv].gr.atm_rx.BLOCKING, cell1, valid);
	   // This should never happen in BLOCKING mode
	   if (valid !== 1'b1) begin
		  $write("ERROR: %M: Rx'ed cell on port #0 is not valid\n");
		  disable input_port;
	   end
	   ATM_CELL[gv].atm_cell.from_bits(cell1);
	   ...
	   begin: region
		  ...
		  // output ports
		  if (cast[gv][0]) begin
			 TX[gv].gt.atm_tx.put_cell(TX[gv].gt.atm_tx.NON_BLOCK,
								cell1, valid);
